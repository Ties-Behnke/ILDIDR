...
\caption{\label{fig:pair_bg_cone} Cones of incoherent $\Pep\Pem$-pairs in the ILD detector for $E_{cms}=250~\GeV$ (a) and $E_{cms}=500~\GeV$ (b)
  ...
  except for the face of the BeamCal and the beam pipe endcap in fron of it (the two leftmost diagonal lines in the plot).}
\end{figure}
%%%%%%%%%%%%%%%%%
...
the TPC, the barrels of Ecal, Hcal, Yoke and the Coil. All other detector dimensions, in particular the longitudenal extents,
and all material thicknesses have not been modified. Thus, the two model have a different aspect ratio.
%
...
\caption{\label{fig:sim_large_small}Left: Integrated radiation lengths of the tracking detectors in the ILD simulation models.
  Right: Material scan in inner tracking region of the simulation model showing detector components of the VTX, SIT and FTD as well
...
real calorimeter, resulting in a very similar material budget in the sensitive part of the layer.
...
better than 1\%, as shown in Fig.~\ref{fig:hybridsim}~(left) for the longitudinal shower profile. Equivalent results have
