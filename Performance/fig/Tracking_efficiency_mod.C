...
//=========  (Wed Jul  3 18:51:05 2019) by ROOT version6.08/06
   TCanvas *c = new TCanvas("c", "efficiency",1625,243,1000,600);
   gStyle->SetOptStat(0);
   ...
   TH1F *Graph_Graph3001 = new TH1F("Graph_Graph3001","",100,0.009,2.199);
   ...
   0,
   3.189816e-05};
   TGraphAsymmErrors *graf = new TGraphAsymmErrors(7,_fx3002,_fy3002,_felx3002,_fehx3002,_fely3002,_fehy3002);
   graf->SetName("");
   graf->SetTitle("");
   graf->SetLineColor(4);
   graf->SetLineWidth(2);
   ...
   TH1F *Graph_Graph3002 = new TH1F("Graph_Graph3002","",100,0.009,2.199);
   ...
   TPaveText *pt = new TPaveText(0.8,0.57,1.327678,0.8,"NB");
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   TText *AText = pt->AddText("ILD");
   pt->Draw();
   
   pt = new TPaveText(1.254581,0.52,2.0,0.80,"NB");
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(42);
...
//   TLegendEntry *entry=leg->AddEntry("NULL","#splitline{e^{+}e^{-}#rightarrow#tilde{#chi}^{+}#tilde{#chi}^{-}#gamma #DeltaM=770 MeV}{#varepsilon (both tracks)}","h");
   TLegendEntry *entry=leg->AddEntry("NULL","e^{+}e^{-}#rightarrow#tilde{#chi}^{+}#tilde{#chi}^{-}#gamma with #DeltaM=770 MeV","h");
