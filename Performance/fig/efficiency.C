...
//=========  (Wed Jul  3 18:51:05 2019) by ROOT version6.08/06
   ...
   Graph_Graph3001->GetYaxis()->SetTitle("Tracking #varepsilon");
   ...
   TPaveText *pt = new TPaveText(0.7165834,0.5797358,1.327678,0.900661,"NB");
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   TText *AText = pt->AddText("ILD");
   pt->Draw();
   
   pt = new TPaveText(1.254581,0.5797358,2.117131,0.8126654,"NB");
   pt->SetFillColor(0);
   pt->SetLineWidth(2);
   pt->SetTextFont(42);
   ...
   entry=leg->AddEntry("","IDR-L #rightarrow 61.44%","p");
   entry->SetLineColor(1);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
   ...
   entry->SetTextFont(62);
   entry=leg->AddEntry("","IDR-S #rightarrow 58.6%","p");
   entry->SetLineColor(1);
   entry->SetLineStyle(1);
   entry->SetLineWidth(1);
