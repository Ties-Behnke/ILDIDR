...
\vspace{1cm}
\subsubsection*{\bf Vertex detector (VTX)}
...
\vspace{1cm}
\subsubsection*{\bf Time projection chamber (TPC)}
...
\end{tabular}
\caption[TPC layout]{Left: Global layout of the TPC. Right: Principle of the ion GEM gating scheme showing the two electric field configurations within (left) and outside (right) bunch trains.}
%\end{figure}
\label{fig:det:TPC}
%\begin{tabular}{cc}
\end{figure}
...
\vspace{1cm}
\subsubsection*{\bf Electromagnetic calorimeter (ECAL)}
%\textit{(Brient, Ootani)}
...
\vspace{1cm}
\subsubsection*{\bf Hadronic calorimeter (HCAL)}
...
\vspace{1cm}
\subsubsection*{\bf Very forward detectors (LumiCAL, LHCAL, BeamCAL)}
...
\end{itemize}
The longitudinal configuration of the very forward detectors has been adapted to the new beam optics as discussed in Chapter 3 (Figure~\ref{ild:fig:Forward_QD0}). The current overall layout of the detectors is shown in Figure~\ref{fig:det:VFS}.  

\begin{figure}[t!]
\centering
\includegraphics[width=0.6\hsize]{Detector/fig/VFS_layout.png}
